...
# cryptography is used to encrypt the message,
# base64, is used for encoding to a text string,
# JSON is used because we are only encrypting the message ,
# os
#########################################################################
import os
import socket
...
def encrypt(key, data, iv):
    ...
    cipher = Cipher(algorithms.AES(bytes(key, encoding='utf-8')), modes.CBC(iv))
...
def listen_for_messages():
    while True:
        ...
        plaintext_message = decrypt(key, obj["msg"].encode('utf-8'), b64decode(obj["iv"]))
        print(f"{obj["colour"]}")
...
while True:
    ...
    #########################################################################
    iv = os.urandom(16)
    to_send = str(encrypt(key, to_send, iv))
    ...
    #########################################################################
    js = {"colour":client_color, "date":date_now, "name":name, "msg":to_send, "iv":b64encode(iv).decode("utf-8")}
    y = json.dumps(js)
